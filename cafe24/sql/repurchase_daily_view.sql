...
-- ※ 비회원(member_id 없음)은 동일인 식별 불가라 제외하고, 미결제·전체취소 주문은 구매 이력에서도 제외한다.
CREATE OR REPLACE VIEW `rf-ads-db-500505.cafe24.rf_cafe24_repurchase_daily` AS
WITH eligible AS (
  SELECT mall, shop_no, member_id, report_date, order_id, ordered_at
  FROM `rf-ads-db-500505.cafe24.rf_cafe24_orders_current`
  WHERE JSON_VALUE(raw_json, '$.paid') = 'T'
    AND JSON_VALUE(raw_json, '$.canceled') != 'T'
    AND member_id IS NOT NULL AND TRIM(member_id) != ''
), seq AS (
  SELECT
