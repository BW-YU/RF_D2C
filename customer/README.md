...
- 같은 워크플로가 적재 직후 시트 **`member_id_1`·`member_id_2` 열을 채운다**(`customer/blocklist_member_id_writeback.py`). 외부 CRM 솔루션이 전화번호 대신 회원 아이디를 요구할 때 그대로 넘기는 용도.
  - **이미 값이 있는 칸은 덮어쓰지 않는다**(슬랙에서 CX가 적어준 아이디를 지우면 안 되므로). 주문 연락처 기준이라 **비회원 주문·주문 이력 없는 번호는 빈칸**(2026-08-18 기준 5,020건 중 1,211건만 매칭, 24%). 커버리지를 올리려면 카페24 어드민 회원정보 엑셀이 필요.
  - 시트를 `rf-mkt` 서비스계정에 **편집자**로 공유해야 동작한다.
- 슬랙 `#1_crm_cx` 의 CRM 차단 요청을 시트에 **자동 적재**한다(`customer/slack_blocklist_sync.py`, 30분마다).
  - 요청 메시지에 연락처·고객명·회원 ID가 이미 들어 있어 그대로 넣는다. BQ 매칭으로는 못 잡는 비회원 주문 건도 커버된다.
  - **클룹·스프린트(자사몰)만** 적재하고 한끼통살·랩노쉬는 건수만 로그에 남긴다.
  - 시트 열은 **헤더 이름으로 찾는다**(`phone`/`member_id_1`/`member_id_2`/`고객명`/`출처`) — 열 순서가 바뀌어도 안 깨진다. 데이터라이즈 등록(체크박스) 열은 건드리지 않는다.
  - 이미 있는 번호는 행을 추가하지 않고 빈 칸만 채운다(멱등). `secrets.SLACK_BOT_TOKEN` 필요(`channels:history` + `chat:write` + `reactions:write`).
  - 적재한 요청에는 ✅ 리액션과 스레드 답글(`N행 추가 / 아이디 …`)을 남긴다. `SLACK_ACK=0` 으로 끌 수 있다.
  - ⚠ **'차단 완료' 버튼은 자동으로 누를 수 없다** — Slack 블록 버튼은 소유 앱에만 이벤트가 가기 때문. 버튼 클릭이 프로세스상 필요하면 사람이 누르거나, 요청 메시지를 만드는 워크플로 소유자가 스텝을 손봐야 한다.
