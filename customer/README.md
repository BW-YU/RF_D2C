...
- 같은 워크플로가 적재 직후 시트 **C열에 `member_id`를 되쓴다**(`customer/blocklist_member_id_writeback.py`). 외부 CRM 솔루션이 전화번호 대신 회원 아이디를 요구할 때 그대로 넘기는 용도.
  - 주문 연락처 기준이라 **비회원 주문·주문 이력 없는 번호는 빈칸**(2026-08-18 기준 5,020건 중 1,211건만 매칭, 24%). 커버리지를 올리려면 카페24 어드민 회원정보 엑셀이 필요.
...
- 슬랙 `#1_crm_cx` 의 CRM 차단 요청을 시트에 **자동 적재**한다(`customer/slack_blocklist_sync.py`, 30분마다).
  ...
  - 시트 열: A=reg B=phone C=member_id_1 D=member_id_2 F=고객명 G=출처(슬랙 링크). E(데이터라이즈 등록 체크박스)는 건드리지 않는다.
