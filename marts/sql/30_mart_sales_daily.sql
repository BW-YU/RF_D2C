...
-- 결제완료 판별: raw_json $.paid='T' AND $.canceled!='T'.
--
...
--    net_sales = SUM(IF(paid, net_charge, 0))/1.1.
--    * actual_order_amount는 환불·취소 후 금액을 담지만 미결제·전체취소에도 잔액이 남을 수 있어 paid 조건으로 제외한다.
...
WITH o AS (
  SELECT report_date, mall, order_id, member_id, payment_amount,
    (JSON_VALUE(raw_json, '$.paid') = 'T' AND JSON_VALUE(raw_json, '$.canceled') != 'T') AS paid,
...
SELECT
  ...
  ROUND(SUM(IF(paid, net_charge, 0)) / 1.1, 2)                       AS net_sales,     -- 순매출(VAT제외, 대시보드 정합)
