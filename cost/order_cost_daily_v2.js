 ...
 * 저장 grain: shipped_date × mall. 월말 주문의 익월 이월을 보존한다.
...
const ENGINE_VERSION = "order-cost-v2.0.0";
  ...
  for (const [date, rows] of byDate) {
    ...
    for (const order of orders.values()) {
      const a = acc.get(order.mall) || { report_date: date, mall: order.mall, net_revenue: 0, cogs: 0, logistics: 0,
        ...
        unmatched_logistics_orders: 0, deal_mapped_lines: 0, deals: new Map() };
      a.order_count++;
...
async function sourceRows(bq, start, end) {
  ...
  SELECT ship_date AS date, i.mall AS mallId, i.order_id AS orderId,
